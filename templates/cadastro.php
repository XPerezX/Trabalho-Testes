<?php
	...
	if (isset($_POST['nome'])){
		...
		// Função que valida os campos para ser adicionado no banco de dados
							...
							<div class="form-group">
								...
								<label for="administrador">Selecione o Administrador:</label>
									...
									// Parte do código que lista os colecionadores do banco de dados 
										include 'db.php';
										$sql = "SELECT registration ,FULLNAME FROM collectors ORDER BY registration"; // Obtém os nomes dos colecionadores 
										$con = mysqli_query($conexao,$sql) or die(mysqli_error($conexao));	// Executa a consulta no banco de dados
										while($dados = mysqli_fetch_assoc($con)) { 	// Obtém os resultados em forma de matriz
											$c = $dados ["FULLNAME"];
